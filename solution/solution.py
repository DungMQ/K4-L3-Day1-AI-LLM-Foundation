...
def estimate_cost(prompt: str, response: str, model: str = OPENAI_MODEL) -> dict:
    ...
    pricing = PRICING_PER_1K_TOKENS.get(
        model, PRICING_PER_1K_TOKENS.get(clean_model, PRICING_PER_1K_TOKENS["gpt-4o"])
    )
    input_cost = (input_tokens / 1000) * pricing["input"]
...
def format_comparison_table(results: list[dict]) -> str:
    ...
    for item in results:
        prompt_txt = item.get("prompt", "")
        gpt4o_txt = item.get("gpt4o_response", "")
        mini_txt = item.get("mini_response", "")

        prompt_fmt = (prompt_txt[:37] + "...") if len(prompt_txt) > 40 else prompt_txt
        gpt4o_fmt = (gpt4o_txt[:37] + "...") if len(gpt4o_txt) > 40 else gpt4o_txt
        mini_fmt = (mini_txt[:37] + "...") if len(mini_txt) > 40 else mini_txt
        gpt4o_lat = f"{item.get('gpt4o_latency', 0.0):.2f}s"
        mini_lat = f"{item.get('mini_latency', 0.0):.2f}s"

        row = f"{prompt_fmt:<40} | {gpt4o_fmt:<40} | {mini_fmt:<40} | {gpt4o_lat:<15} | {mini_lat:<15}"
        lines.append(row)
